...
def load_unlabelled_data_marked(config):
    ...
    unlabelled_df["feature_text"] = unlabelled_df[["feature_text", "case_num"]].apply(
...
def get_unlabelled_dataset_marked(config):
    ...
    df = load_unlabelled_data_marked(config)
    ...
    dataset_creator = NbmeMTLDatasetNeoMarked(config)
...
def get_unlabelled_dataloader_marked(config):
    """get unlabelled dataloader for training MPL

    :param config: mpl config
    :type config: dict
    """
    unlabelled_ds = get_unlabelled_dataset_marked(config)
    ...
    # DebertaV2TokenizerFast.from_pretrained(config["base_model_path"])
    # add tokens
...
def get_train_valid_dataloaders_marked(config):
    """get train and valid dataloader with markers added
    ...
    nbme_data = DataProcessor.process_data(nbme_data, add_markers=True)
    ...
    print(f"size of valid: {valid_df.shape}")
...
def execute_meta_training(config):
    ...
    unlabelled_dl = get_unlabelled_dataloader_marked(config)
    train_dl, valid_dl = get_train_valid_dataloaders_marked(config)
