...
config["dropout"] = 0.1
config["num_layers_reinit"] = 12
config['scheduler_name'] = 'OneCycleLR'
config["lr"] = 2e-5
config["weight_decay"] = 1e-4
config["epochs"] = 5
config["num_accumulate_grad"] = 4
config["val_check_interval"] = 0.5
config["patience"] = 10
config["encoder_lr"] = 1e-5
config["decoder_lr"] = 2e-5
...
config["batch_size"] = 8
config['max_length'] = 470

config["pct_start"] = 0.025
config["div_factor"] = 100
config["final_div_factor"] = 100
config["three_phase"] = False
config["anneal_strategy"] = "cos"
config["arch"] = "deberta"
config["eps"] = 1e-6
...
config['steps_per_epoch'] = int(len(train_dl)/config['num_accumulate_grad'])+1
...
model = NbmeModelMTLNeo(config, scorer=scorer_fn)
...
def get_callbacks(config):
    ...
    swa_callback = StochasticWeightAveraging(
        swa_lrs=1e-5, swa_epoch_start=2, annealing_epochs=5
    )
